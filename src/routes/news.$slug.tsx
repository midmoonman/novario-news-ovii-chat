...
function ArticlePage() {
  ...
  return (
    ...
    >
      {/* Background blobs for glassmorphism effect - SAME AS HOME */}
      ...
      <div className="relative z-10 flex flex-col min-h-screen">
        ...
        <main className="mx-auto max-w-4xl w-full px-4 pt-10 pb-20 flex-1">
          <motion.article 
            ...
            className="glass-2 rounded-[48px] p-6 md:p-12 border border-white/5 shadow-2xl"
          >
            <div className="flex items-center gap-3 text-[10px] uppercase tracking-[0.3em] mb-6 font-black">
              <span className="bg-primary text-primary-foreground rounded-full px-4 py-1.5 shadow-glow">{article.category}</span>
              <span className="text-white/40">{article.publishedAt} · {article.readTime} min read</span>
            </div>
            <h1 className="serif text-4xl md:text-6xl font-black leading-[1.1] text-balance tracking-tight">{article.title}</h1>
            <p className="mt-8 text-xl text-white/50 leading-relaxed font-medium">{article.excerpt}</p>

            <div className="mt-10 flex items-center justify-between gap-3 pb-8 border-b border-white/5">
              <div className="flex items-center gap-4">
                <div className="h-12 w-12 rounded-full glass-2 flex items-center justify-center font-black text-primary text-xl border border-white/10">
              ...
              <a href={article.url} target="_blank" rel="noopener noreferrer" className="text-[11px] uppercase tracking-widest font-black text-primary hover:opacity-70 transition-opacity">Read source ↗</a>
            </div>

            <motion.div 
              className="my-12 rounded-[32px] overflow-hidden shadow-2xl border border-white/5"
            ...
            <div className="max-w-none text-[19px] leading-[1.8] text-white/80 font-medium font-serif space-y-6">
            ...
            <div className="mt-16 pt-10 border-t border-white/5 flex items-center justify-between">
              <div className="flex gap-4">
                <button 
                  ...
                  className="rounded-full glass-2 border border-primary text-primary hover:bg-primary hover:text-primary-foreground px-10 py-3 transition-all font-black uppercase tracking-widest text-[11px]"
                >
                  Share Story
                </button>
              </div>
